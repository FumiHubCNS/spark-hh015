...
TIME_RANGE_NS = (-100, 300) # valid time range for SRPPAC strip hits after anode time subtraction and ns2ns conversion
HIT_SIZE_LIMIT = 6 # maximum number of hit strips per event to accept
HALF_STRIP_SIZE = (2.55 / 2.0, 2.58 / 2.0) # [x,y] mm
CENTER_ID = (16, 16) # [x,y] strip id
CALIB_RUNNAME = 'run1083'
...
def decode_srppac_amaneq(spark: SparkSession, df: DataFrame, preamp_type: str) -> DataFrame:
    ...
    df_prm_x = spark.read.csv(f'/home/h487/notebooks/jan2026/kojima/srppac_koji/prm/srx_charge_calib_{preamp_type}.csv', inferSchema = True, header = True)
    df_prm_y = spark.read.csv(f'/home/h487/notebooks/jan2026/kojima/srppac_koji/prm/sry_charge_calib_{preamp_type}.csv', inferSchema = True, header = True)
...
def calib_srppac_dqdx(spark: SparkSession, df: DataFrame) -> DataFrame:
    ...
    for i, dfxy in enumerate(dfs):
        ...
        # Monotone converter
        df_conv = spark.read.csv(f"/home/h487/notebooks/jan2026/kojima/srppac_koji/prm/srppac_q0q1_{planes[i]}_{CALIB_RUNNAME}.csv",inferSchema=True,header=True)
        ...
        # Calculate position
        # Calculate position
        dfp = dfp.withColumn("ins", F.expr("CASE WHEN id1 = id0 THEN 0 WHEN id1 = id0 + 1 THEN -ins ELSE ins END"))
        ...
        dfp = dfp.select("hbfNumber","id0","pos","ins").withColumnRenamed("id0",f"sr_id0_{planes[i]}").withColumnRenamed("pos",f"sr_pos_{planes[i]}").withColumnRenamed("ins",f"sr_ins_{planes[i]}")
...
#    strip_count = df_srppac.filter('charge2_x > 0 AND charge2_y > 0 AND ABS(id0_x - id1_x) = 1 AND ABS(id0_y - id1_y) = 1 AND ABS(id0_x - id2_x) = 1 AND ABS(id0_y - id2_y) = 1').count()
    strip_count0 = df_srppac.filter('charge0_x > 0 AND charge0_y > 0').count()
    if preamp_type == "asagi":
        strip_count1 = df_srppac.filter('charge1_x > 0 AND charge1_y > 0 AND ABS(id0_x - id2_x) = 1 AND ABS(id0_y - id2_y) = 1').count()
    else:
        strip_count1 = df_srppac.filter('charge1_x > 0 AND charge1_y > 0 AND ABS(id0_x - id2_x) = 1 AND ABS(id0_y - id2_y) = 1').count()
    print(f"SRPPAC strip>0 data count: {strip_count0}")
    print(f"SRPPAC strip>1 data count: {strip_count1}")
    if sra_count:
        print(f"SRPPAC T0 efficiency: {strip_count0/sra_count*100:.2f} %")
        print(f"SRPPAC T1 efficiency: {strip_count1/sra_count*100:.2f} %")
    else:
        print("sra_count = 0")
    # Calibrate SRPPAC data
    ...
    if args.output_file:
        ...
        ofname = stem + ("_srppac_koji.parquet")
