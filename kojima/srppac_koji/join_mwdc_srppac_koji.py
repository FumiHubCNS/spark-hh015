...
if __name__ == "__main__":
    ...
    # Open parquet File
    srppac_fname = args.input_prefix + "_srppac_koji.parquet"
    mwdc_fname = args.input_prefix + "_mwdc_koji.parquet"
    df_srppac = spark.read.parquet(srppac_fname)
    df_srppac = df_srppac.select(["hbfNumber"]+[c for c in df_srppac.columns if c.endswith("_pos_x") or c.endswith("_pos_y") or c.endswith("_id0_x") or c.endswith("_id0_y")])
    df_mwdc = spark.read.parquet(mwdc_fname)
    df_mwdc = df_mwdc.select(["hbfNumber"]+[c for c in df_mwdc.columns if c.endswith("_x") or c.endswith("_y") or c.endswith("_id0")])
    df = df_srppac.join(
        df_mwdc,
        on=["hbfNumber"]
    )
    ...
    if args.output_file:
        ...
        ofname = args.input_prefix + ("_joined_koji.parquet")
    df.printSchema()
